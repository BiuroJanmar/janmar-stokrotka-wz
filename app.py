...
def sprawdz_haslo():
    ...
    st.title("🔐 Autoryzacja systemu JANMAR")
    st.subheader("Dostęp zastrzeżony dla pracowników firmy")
    
    haslo_wpisane = st.text_input("Wprowadź hasło dostępowe:", type="password")
    
    # TUTAJ MOŻESZ ZMIENIĆ HASŁO NA SWOJE
    if st.button("Zaloguj się"):
        if haslo_wpisane == "Janmar2026!":
            st.session_state["zalogowany"] = True
            st.rerun()
        else:
            st.error("❌ Niepoprawne hasło. Dostęp zabroniony.")
            
    return False

# Jeśli hasło jest niepoprawne, zatrzymujemy działanie aplikacji tutaj
if sprawdz_haslo():

    # ==============================================================================
    # WŁAŚCIWA APLIKACJA (URUCHAMIA SIĘ TYLKO PO ZALOGOWANIU)
    # ==============================================================================
    st.title("JANMAR WZ-Stokrotka Web v10.0 🔒")
    st.subheader("Oficjalny, zabezpieczony generator dokumentów WZ")
    st.write("Wgraj surowy plik tekstowy (.TXT) zamówienia ze Stokrotki.")

    # Przycisk do wylogowania dla bezpieczeństwa
